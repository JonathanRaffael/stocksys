...
const { PrismaClient, Role, Shift, Prisma } = prismaPkg
...
router.get("/", async (req, res) => {
  try {
    const schema = z.object({
      ...
      includeProduct: z.coerce.boolean().optional(),
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (role === "IPQC") {
        if (typeof parsed.beforeIpqc === "number") updateData.beforeIpqc = parsed.beforeIpqc
        if (typeof parsed.afterIpqc === "number") updateData.afterIpqc = parsed.afterIpqc
        if (typeof parsed.onGoingPostcured === "number") updateData.onGoingPostcured = parsed.onGoingPostcured
        if (typeof parsed.afterPostcured === "number") updateData.afterPostcured = parsed.afterPostcured
      } else {
        if (typeof parsed.beforeOqc === "number") updateData.beforeOqc = parsed.beforeOqc
        if (typeof parsed.afterOqc === "number") updateData.afterOqc = parsed.afterOqc
        if (typeof parsed.onHoldOrReturn === "number") updateData.onHoldOrReturn = parsed.onHoldOrReturn
      }
