...
exports.ipqcSummaryRouter = exports.summaryRouter = void 0;
// @ts-nocheck
const express_1 = require("express");
const client_1 = __importDefault(require("@prisma/client"));
const zod_1 = require("zod");
const date_fns_1 = require("date-fns");
const { PrismaClient, Shift } = client_1.default;
const prisma = new PrismaClient();
/** WIB = UTC+7 */
const WIB_OFFSET_MS = 7 * 60 * 60 * 1000;
const toUtcFromWib = (d) => new Date(d.getTime() - WIB_OFFSET_MS);
const rangeUtcFromLocalWib = (ymd) => {
    const local = new Date(`${ymd}T00:00:00`);
    const startUtc = toUtcFromWib((0, date_fns_1.startOfDay)(local));
    const endUtc = toUtcFromWib((0, date_fns_1.endOfDay)(local));
    return { startUtc, endUtc };
};
// ================= Routers =================
exports.summaryRouter = (0, express_1.Router)(); // -> /api/summary
exports.ipqcSummaryRouter = (0, express_1.Router)(); // -> /api/ipqc/summary
/* ============================================================================
...
exports.summaryRouter.get("/", async (req, res) => {
    try {
        ...
        const defaultFrom = from ?? new Date(todayLocal.getTime() - 6 * 86400000).toISOString().slice(0, 10);
        ...
        // KPI hari ini (WIB)
        const { startUtc: startTodayUtc, endUtc: endTodayUtc } = rangeUtcFromLocalWib(new Date().toISOString().slice(0, 10));
        const [totalProducts, activeProducts, totalUsers, todayByShiftRaw, lastEntriesRaw, grouped,] = await Promise.all([
        ...
        const todayByShift = { S1: 0, S2: 0, S3: 0 };
        ...
        const perProduct = grouped
            ...
            return {
                ...
                totalAfterOqc,
                totalHoldOrReturn,
                totalAvailable: totalAfterIpqc + totalAfterPostcured + totalAfterOqc - totalHoldOrReturn,
            };
        })
 ...
 * ==========================================================================*/
const IpqcQuery = zod_1.z.object({
    date: zod_1.z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
    shift: zod_1.z.nativeEnum(Shift),
    plant: zod_1.z.string().optional(),
    line: zod_1.z.string().optional(),
});
exports.ipqcSummaryRouter.get("/summary", async (req, res) => {
    try {
        res.setHeader("Cache-Control", "no-store");
        ...
        const passRateIpqc = totalBeforeIpqc > 0 ? Math.round((totalAfterIpqc / totalBeforeIpqc) * 100) : 0;
        const denomPostcure = totalAfterIpqc + totalOnGoingPostcured + totalAfterPostcured;
        const passRatePostcure = denomPostcure > 0 ? Math.round((totalAfterPostcured / denomPostcure) * 100) : 0;
